...
$(function(){
    $(document).ready(function(){
        $.get("http://192.168.0.120:8080/finalproject/user/following", {
          ...
          var all = "";
          var following_num = followings.length;
      ...
      for(var i=0;i<exps.length;i++){
        if(exps[i].rank==rank){
          ...
          $(points).html(point);
          $(follow_num).html("您共关注了"+following_num+"位UP主");
          $(rankid).html("等级："+rank);
      $(expid).html("经验值："+exp+"/"+exptop);

          $.each(followings, function (index, item) {
            var username = followings[index].username;
            ...
            if(resourses.length==0){
                ...
                '<div class="__List__Left">'+
                  ...
                  '<div class="__List__Info">'+
                    '<h3>'+ username +'</h3>'+
                    '<a><em>创作 '+ upload_num +'</em> | <em>粉丝 ' + follow_num +'</em></a>'+
                    '<button type="button" id='+ "cancel"+index+' class="__GZ__">取消关注</button>'+
                  '</div>'+
                '</div>'+
      ...
      var GenerateCss = function () {
        ...
        var btnOk = function (callback, rid) {
          $("#mb_btn_ok").click(function () {
              ...
              check = "yes";
              var followingid = followings[rid].username;
